...
async function aiEvolve() {
  ...
  // Trade detail — BOTH winning and losing
  ...
  const antiSpiralWarning = allTighten
    ? '\n⚠️ 严重警告：过去3代都在收紧！这是负反馈螺旋！本次必须反向操作：提高权重、关闭不必要的过滤器。目标是找到能赚钱的信号，不是消灭所有信号。'
    : (allLoosen ? '\n⚠️ 提示：过去3代都在放松，注意不要过度激进。' : '');

  const prompt = `你是比特币交易策略优化专家。威廉姆斯混沌操作法，200倍杠杆，双向交易，模拟盘。
...
返回纯JSON:
{"analysis":"一句话","paramChanges":{"param":value},"enableFilters":[],"disableFilters":[],"weightChanges":{"ruleType":0.8}}`;
      ...
      for (const k in changes.weightChanges) {
        for (const cat of ruleCategories) {
          ...
          if (rule) {
            ...
            const floor = isEntry ? 0.5 : 0.1;
            rule.weight = +Math.max(floor, Math.min(2.0, changes.weightChanges[k])).toFixed(2);
            if (rule.weight < oldW) tightenCount++; else if (rule.weight > oldW) loosenCount++;
